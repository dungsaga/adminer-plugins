...
* `mark-repeated-values.php`: Use alt+click to highlight the same values in other cells of a column (up to 4 colors)
* `color-for-column-type.php`: Show different color for each column type (number, datetime, enum ...)

# screenshots

Here are some values marked with colors in sticky columns and in normal columns:
...
Here are row values colored according to their data types (and the table list in left sidebar is filtered with a regex):
